...
public record HookSnapshot(
    ...
    public static HookSnapshot fromResult(com.justnothing.testmodule.protocol.json.response.HookListResult result) {
        List<HookItem> items = new ArrayList<>();
        if (result.getHooks() != null) {
            for (com.justnothing.testmodule.protocol.json.response.HookListResult.HookItem d : result.getHooks()) {
