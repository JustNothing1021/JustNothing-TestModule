package com.justnothing.testmodule.ui.analysis.exportcontext;
...
public class ExportContextQueryViewModel extends AndroidViewModel {

    private static final String TAG = "ExportCtxQueryVM";
    ...
    public void queryContext() {
        ...
        executor.execute(() -> {
            try {
                ...
                if (parsedResult instanceof ExportContextResult result) {
                    if (result.isSuccess() && result.getFields() != null) {
                        ...
                        String errorMsg = result.getError() != null
                                ...
                                : getApplication().getString(R.string.analysis_export_context_failed);
                        logger.error("查询失败: " + errorMsg);
                        error.postValue(errorMsg);
                    }
                } else {
                    String errorMsg = getApplication().getString(R.string.analysis_response_type_error,
                            "ExportContextResult", parsedResult.getClass().getName());
                    logger.error(errorMsg);
                    error.postValue(errorMsg);
                }
            } catch (Exception e) {
                logger.error("查询异常", e);
                error.postValue(getApplication().getString(R.string.analysis_export_context_failed) + ": " + e.getMessage());
            } finally {
