...
/**
 * jank 各子命令的公共基类：分档上色、组帧、纯文本降级。
 *
 * <h3>为什么这里要写两套输出</h3>
 * <p>{@code jank} 大概率是被人盯着看的（Live 刷新的一屏仪表盘），但也可能被 agent 调、被
 * 重定向到文件。后两种场景没有终端尺寸、也没有 ANSI 能力，{@link #buildFrame} 生成的东西
 * 会变成一堆空白。所以"能上色就上色、不能就一行纯文本"是硬要求，
 * 判断入口统一是 {@link CommandExecutor.CmdExecContext#supportsRichOutput()}。</p>
 *
 * <h3>关于布局预算</h3>
 * <p>每个面板的高度都由内容决定（左列是一个 {@code Group}，不是 {@code splitRow} 等分），
 * 内容超了会在整列底部被裁掉。所以 TOP / 流水这两个"行数可变"的榜单必须按屏幕高度
 * 现算长度，见 {@link #topBudget} / {@link #churnBudget}。</p>
 */
public abstract class AbstractJankCommand<Req extends CommandRequest<?>>
    ...
    /**
     * CPU / 队列的分档：60% 以下正常，85% 以上就说明这一秒有东西在排队了。
     *
     * <p>阈值定在 85 而不是 90：手表只有四个核，一个核被跑满就少掉 25% 的算力，
     * 等到总体 90% 才标红，卡顿早就发生了。</p>
     */
    ...
    /** 内存分档和 CPU 不一样：占用 80% 完全正常（page cache 会顶上去），95% 才开始危险。 */
    ...
    /** 用某个 style 包住一段文本，RichConsole 的 markup 语法。 */
     ...
     * 一条百分比进度条。
     *
     * <p>用 {@link ProgressBar} 而不是自己拼方块字符：它自带宽度感知和样式合并，
     * 自己拼的话宽度和颜色都要手算，还得担心终端窄了会折行。这里它是"独立可渲染对象"，
     * 不是挂在 {@code Progress} 上刷新的那种 —— 每帧重建一个新的，
     * 因为整帧本来就在重建。</p>
     */
     ...
     * 指定宽度的进度条。
     *
     * <p>右列只有屏幕的三分之一宽，面板内部还要放三列（标签/条/值），
     * 用左列那么宽的条会把值那一列挤到只剩 4 个字符 —— 然后 {@code 0.8/核} 这种
     * 稍微长一点的值就折行，一折行整帧高度就变了，布局预算全废。</p>
     */
     ...
     * 把一帧画成整屏仪表盘。
     *
     * <p>左列三块是"这一瞬间机器怎么样"（CPU / 内存 / 谁在吃 CPU），
     * 右列两块是"这段时间发生了什么"（进程流水 / 采样状态）。</p>
     *
     * <p>两列都是"一个 leaf Layout 包一个 Group"，而不是 {@code splitRow} 去等分高度：
     * 等分的语义是固定区域、内容超了就裁，24 行屏幕切成三份后每个面板只有 8 行，
     * 表格十几行必然断头，看起来就像是边框跑到别的地方去了。</p>
     */
    ...
    private static Table cpuTable(JankFrame frame, int cores) {
        ...
        if (!frame.cpuOk) {
            table.addRow(style("red", "读不到 /proc/stat"), "", "");
            return table;
        }
        if (frame.baseline) {
        ...
        table.addRow("总", bar(frame.busyPct), coloredPct(frame.busyPct, levelCpu(frame.busyPct)));
    ...
    private static Table memTable(JankFrame frame) {
        ...
        if (!frame.memOk) {
            table.addRow(style("red", "读不到 /proc/meminfo"), "", "");
            return table;
        }
        ...
        if (frame.swapOk) {
            table.addRow("换出", bar(frame.swapUsedPct),
     ...
     * CPU 占用榜单。
     *
     * <p>口径与 {@code top} 一致：相对<b>单核</b>。四核机上跑满一个核就是 100%，
     * 四个核都跑满会显示 400% —— 这不是 bug，是把"一个线程把机器吃掉了"这件事放大到看得见。</p>
     */
    private static Table topTable(JankFrame frame, int topRows) {
        ...
        if (!frame.procOk) {
            table.addRow(style("dim", "—"), style("red", "读不到 /proc（hidepid？）"));
            return table;
        }
        if (frame.baseline) {
        ...
        for (JankFrame.TopEntry entry : frame.topCpu) {
            ...
            table.addRow(style(levelCpu(entry.pct), pctText), entry.name);
            rows++;
        }
        return table;
    }
     ...
     * 让单元格"不折行、超了打省略号"。
     *
     * <p>这是这里最关键的一行。表格默认会<b>折行</b>，而折行会凭空多占一行高度 ——
     * 整个布局预算是按"每行数据占一行"算的，一折行就把最下面的状态面板顶出屏幕。
     * 换成省略号之后，宽度由表格自己按实际可用空间算，我们既不用猜"这一列到底多宽"，
     * 也不用在采样前把名字截短（在宽终端上白白切掉信息）。</p>
     */
    ...
    /**
     * 采样状态：进度、规模、排队压力、这段时间都花在哪、采集自身开销。
     *
     * <p><b>每个值都必须塞得进 4 个字符</b>。右列只有屏幕三分之一宽，面板内部三列
     * 平分之后值那一列就这么宽 —— 写 {@code 0.8/核} 或者 {@code u30 s21 io7} 一定会折行，
     * 折行多出来的行会把整帧顶出屏幕。所以这里：条只画 6 格、采样只报百分比、
     * CPU 时间分布拆成三行、单位一律不进值（放标签里）。</p>
     *
     * <p>把"队列"和"采样耗时"放进来的理由：前者是卡顿的直接证据（每核排了几个），
     * 后者是采样器自己的清白证明 —— 如果采集一帧要 300ms，测出来的卡顿就是它自己造成的。</p>
     */
    ...
    /**
     * 一帧的布局预算：屏幕高度决定"每块放几行"。
     *
     * <p>行数超了会被在整屏底部裁掉（列是 {@code Group}，高度由内容决定），
     * 状态面板直接消失，所以这个数必须算准。</p>
     *
     * <p>宽度<b>不</b>在这里管：单元格统一设成"不折行 + 省略号"（见 {@link #noWrap}），
     * 由表格自己按实际可用空间省略。早先这里有一版按屏幕宽度估算"名字该截几个字"的代码，
     * 因为要猜表格边框/padding 到底吃掉多少格，估错一格就会出现一次折行 —— 直接交给表格更省事。</p>
     */
    protected static class Budget {
        ...
        /**
         * @param screenHeight 终端行数（拿不到就传 24）
         * @param cores        CPU 核数
         * @param wantedTop    TOP 榜单期望长度
         * @param wantedChurn  流水期望条数
         */
        public static Budget of(int screenHeight, int cores, int wantedTop, int wantedChurn) {
            ...
            int topRows = clamp(screenHeight - (16 + cores) - 1, 1, wantedTop);
            // 右列 = 流水面板（2 + 2 + 行数）+ 状态面板（2 + 2 + 9 行）
            int churnRows = clamp(screenHeight - (4 + 13) - 1, 1, wantedChurn);
            return new Budget(topRows, churnRows);
        }
    }

    /** 夹在区间里的整数，别让调用方到处写 Math.max/min。 */
     ...
     * 一帧压缩成一行。
     *
     * <p>没有富渲染能力时这是唯一的输出形式，所以信息不能少：CPU 总占比、各核、内存、
     * 队列、进程/线程数、D 状态、采样耗时。有富渲染时它也还是留着用处 ——
     * 采样结束后能一眼扫过整段时间的变化。</p>
     */
    ...
    /** kb → 人类可读。手表上的数字都在 MB 量级，到 GB 就不用小数了。 */
