...
public class BytecodeAnalyzeRequest extends CommandRequest<BytecodeResult> {

    @CmdParam(
        name = "className",
        position = 1,
        required = true,
        description = BytecodeTexts.PARAM_BYTECODE_ANALYZE_CLASSNAME_DESC
    )
    ...
    @CmdParam(
        ...
        aliases = {"-v", "--verbose"},
        required = false,
        description = BytecodeTexts.PARAM_BYTECODE_ANALYZE_VERBOSE_DESC
    )
    private boolean verbose;

    public BytecodeAnalyzeRequest() {
        super();
    }
