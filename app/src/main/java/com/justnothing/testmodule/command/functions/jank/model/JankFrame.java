...
public class JankFrame {
    ...
    public static class TopEntry {
        ...
        /** 内核 comm，最多 15 个字符：统计口径用它（短、稳定、不随 cmdline 变）。 */
        public final String name;
        /** 面板上显示的名字：cmdline 全名；取不到时等于 {@link #name}。 */
        public final String displayName;
        public final double pct;

        public TopEntry(int pid, String name, double pct) {
            this(pid, name, pct, name);
        }

        public TopEntry(int pid, String name, double pct, String displayName) {
            this.pid = pid;
            this.name = name;
            this.pct = pct;
