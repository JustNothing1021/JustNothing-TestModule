...
public class WatchMain {
    ...
    public static class WatchCmdLogger extends Logger {
        @Override
    ...
    public static final WatchCmdLogger logger = new WatchCmdLogger();
    ...
    public static String getHelpText() {
        return String.format("""
                ...
                子命令:
                    add <field|method> <class_name> <member_name> [sig/signature <signature>] [间隔]  - 添加watch任务
                    list                                         - 列出所有watch任务
                    stop <id>                                    - 停止指定watch
                    clear                                        - 清除所有watch
                    output <id> [数量]                            - 获取指定watch的输出
                    output all [数量]                             - 获取所有watch的输出
    ...
    private static String handleAdd(String[] args, ClassLoader classLoader, WatchManager manager) {
        if (args.length < 4) {
    ...
    private static String handleList(WatchManager manager) {
        return manager.listWatches();
    }

    private static String handleStop(String[] args, WatchManager manager) {
        if (args.length < 2) {
            return "参数不足\n用法: watch stop <id>\n" + getHelpText();
        }
    ...
    private static String handleClear(WatchManager manager) {
        int count = manager.getWatchCount();
        manager.clearAll();
        return "已清除所有watch任务，共 " + count + " 个";
    }

    private static String handleOutput(String[] args, WatchManager manager) {
        if (args.length < 2) {
            return  """
                    参数不足
