...
import com.justnothing.testmodule.command.functions.agent.handlers.AgentCommandHandler;
import com.justnothing.testmodule.command.functions.agent.router.AgentCommandRouter;
import com.justnothing.testmodule.command.functions.agent.handlers.DbListHandler;
import com.justnothing.testmodule.command.functions.agent.handlers.DbQueryHandler;
import com.justnothing.testmodule.command.functions.agent.handlers.DbTablesHandler;
import com.justnothing.testmodule.command.functions.agent.handlers.SpListHandler;
import com.justnothing.testmodule.command.functions.agent.handlers.SpReadHandler;
import com.justnothing.testmodule.command.functions.agent.handlers.SpWriteHandler;
...
public class InspectionAgent {
        ...
        try (client) {
            try {
                ...
                if ("_dispatch".equals(commandType)) {
                    ...
                    // 目标进程没有自己的终端：渲染要用客户端真实能力（宽高 / ANSI），
                    // 由 CLI 侧在 _dispatch 里透传过来。缺了它渲染出来就是空的。
                    ClientRequirements clientReq = ClientRequirements.fromRpcParamsJson(params.optString("client", null));

                    // 使用当前 Socket 的流进行交互式执行
                    executeDispatchedCommandInteractive(cmdStr, clientReq,
    ...
    private void executeDispatchedCommandInteractive(String commandStr,
        ...
        try {
            ...
            // 关键一步：把客户端的真实能力应用进来。不调的话 RemoteServerTerminal 会是
            // TYPE_DUMB + 0x0 尺寸 —— RichConsole 渲染出来是空字符串，
            // 表现就是"命令跑成功了，但代码/进度条什么都没有"。
            if (clientReq != null) {
                outputHandler.applyClientRequirements(clientReq);
            } else {
                // CLI 没给能力 = 它自己也没有通往客户端的渲染通道（GUI / JSON 模式）。
                // 那种情况下我们的渲染结果发出去没人接，必须提前降级成纯文本，
                // 否则用户看到的就是"命令成功但内容凭空消失"。
                outputHandler.setDownstreamReachable(false);
            }
            // 代理会话本质是交互式的，输入必须开着（applyClientRequirements 可能按客户端能力把它关掉）
            ...
            try {
                ...
                // 能力优先用 CLI 透传来的；拿不到就退回"只支持输入"的保守值
                ClientRequirements requirements = clientReq != null
                        ? clientReq
