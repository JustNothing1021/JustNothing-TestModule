...
public class RichDemoMain extends MainCommand<CommandResult> {
    ...
    protected CommandResult executeInternal(CommandExecutor.CmdExecContext<CommandRequest<?>> context) throws Exception {
        ...
        // =====================================================================
        console.rule("Rules");
        console.rule("Left aligned", cfg -> cfg.style("green").align("left"));
        console.rule("Right aligned", cfg -> cfg.style("cyan").align("right"));
        console.println();
        ...
        console.println(Syntax.of(pythonCode, cfg -> cfg.lexerName("python").lineNumbers(true).highlightLines(new HashSet<>(Arrays.asList(8)))));
        console.println();
