...
public class JankSampler {
     ...
     * 恰恰把最该被看见的那种进程给缩小了。</p>
     */
    private static void diffProcesses(JankFrame frame, Map<Integer, ProcessEntry> prevIndex,
                                      Map<Integer, ProcessEntry> curIndex,
     ...
     *
     * <p>消失事件用<b>上一帧</b>记下的名字：进程一死 {@code /proc/<pid>} 就没了，
     * 这时候再去读名字只会读到 null，流水里就只剩一串光秃秃的 pid。</p>
     */
    private void updateChurn(ProcSnapshot prev, ProcSnapshot cur,
        ...
        for (Map.Entry<Integer, ProcessEntry> e : curIndex.entrySet()) {
            if (!prevIndex.containsKey(e.getKey())) {
                push(new ProcChurn(true, e.getKey(), name(e.getValue())));
                changed = true;
            }
        }
        for (Map.Entry<Integer, ProcessEntry> e : prevIndex.entrySet()) {
            if (!curIndex.containsKey(e.getKey())) {
                push(new ProcChurn(false, e.getKey(), name(e.getValue())));
                changed = true;
            }
        }
        if (changed) {
    ...
    private static String name(ProcessEntry entry) {
        if (entry == null) {
            return "?";
        }
