 ...
 *     new DirectClassCommand<>("hierarchy",
 ...
 *     new DirectClassCommand<>("setfield",
...
public class DirectClassCommand<Req extends ClassCommandRequest<?>, Res extends ClassCommandResult>
     ...
     */
    public DirectClassCommand(String commandName,
    ...
    protected Res executeClassCommand(ClassCommandContext<Req> context) throws Exception {
        try {
            ...
            if (result == null) {
                logger.warn("DirectClassCommand " + commandName + " 返回了null结果");
                result = createErrorResult("命令执行返回null", responseType);
            }

            return result;
        } catch (Exception e) {
            logger.error("DirectClassCommand " + commandName + " 执行失败", e);
            throw e;
        }
    }
}
