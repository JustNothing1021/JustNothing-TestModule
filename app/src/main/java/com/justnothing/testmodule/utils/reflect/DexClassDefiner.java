...
public class DexClassDefiner implements ClassDefiner {
    ...
    /**
     * 当前装载器链的尾节点（充当下一个新装载器的 parent）。
     * <p>
     * 生成的类之间会互相引用，最典型的是<b>匿名类继承脚本声明的类</b>：
     * {@code class B { ... } new B() { ... }} 里匿名类的父类就是另一个生成类。
     * 如果每个类都用一个全新的 DexClassLoader，后定义的类在父链上找不到先定义的类，
     * 加载时报 {@code ClassNotFoundException: anon$B$4}。串成一条链即可互相可见。
     */
    private ClassLoader chainTail;
    /** 链对应的 parent，换了 parent 就另起一条链，避免把不同宿主的类混在一起。 */
    private ClassLoader chainParent;
    ...
    public Class<?> defineClass(String name, byte[] bytecode, ClassLoader parent) throws Exception {
        ...
        ClassLoader previousTail;
        synchronized (this) {
            if (chainTail == null || chainParent != effectiveParent) {
                chainParent = effectiveParent;
                chainTail = effectiveParent;
            }
            previousTail = chainTail;
        }

        ClassLoader dexClassLoader = null;
        try {
            dexClassLoader = createDexClassLoader(dexFile, previousTail);
            LOADER_CACHE.put(loaderKey, dexClassLoader);
            synchronized (this) {
                chainTail = dexClassLoader;
            }
            return dexClassLoader.loadClass(name);
        } catch (Exception e) {
            cleanupDexFile(dexFile);
            // 装载器没建成/没加载成功就不要留在链上，否则后续类会挂着一个坏 parent
            synchronized (this) {
                if (chainTail == dexClassLoader) {
                    chainTail = previousTail;
                }
            }
            throw e;
        }
