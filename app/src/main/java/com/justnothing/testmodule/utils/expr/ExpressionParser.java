package com.justnothing.testmodule.utils.expr;
...
public final class ExpressionParser {
    ...
    private static Object evaluateExpression(String expression, ClassLoader classLoader, List<String> imports) {
        ...
        // executeWithResult 在脚本模式下会内部重置会话（ParseContext + 变量），
        // 会把下面配置的 import 冲掉。这里自己控制重置时机：先重置，再配置 import，
        // 随后执行阶段不再重置 —— 每次求值的隔离仍由这次 resetSession 保证。
