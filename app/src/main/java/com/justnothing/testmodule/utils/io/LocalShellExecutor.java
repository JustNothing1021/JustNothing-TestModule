...
public class LocalShellExecutor implements ShellExecutor {
    ...
    // Android 特有路径上的系统 shell；仅在 Android 上存在
    private static final boolean IS_ANDROID =
            new File("/system/bin/sh").exists() && !IS_WINDOWS;

    // 类 Unix 环境下的 shell：Android 用 /system/bin/sh，桌面 Linux/macOS 用 /bin/sh
    private static final String UNIX_SHELL = IS_ANDROID ? "/system/bin/sh" : "/bin/sh";
    ...
    public IOManager.ProcessResult execute(String command, long timeoutMs) throws ShellExecutionException {
        ...
        try {
            String[] cmdArray = IS_WINDOWS
                    ...
                    : new String[]{UNIX_SHELL, "-c", command};
    ...
    public String getDescription() {
        String shell = IS_WINDOWS ? "cmd" : UNIX_SHELL;
