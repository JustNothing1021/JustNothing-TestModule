...
public class RootShellExecutor implements ShellExecutor {
    ...
    public IOManager.ProcessResult execute(String command, long timeoutMs) throws ShellExecutionException {
        ...
        } catch (RootCommandTimeoutException e) {
            // 超时不等于线程被中断：这里绝不能 re-interrupt，否则调用线程的中断标志会被置上，
            // 同一条线程后续的每一次阻塞调用（BlockingQueue.poll / Thread.sleep…）都会立刻失败
            // ——比如 DataDirectoryManager 紧接着在本地做的 fallback。binder 线程还会带着这个
            // 标志回到线程池，污染之后完全不相关的请求。
            throw new ShellExecutionException(command, ShellType.ROOT, e.getMessage(), e);
