...
public class ThreadPoolManager {
    ...
    private Runnable wrapTask(Runnable task) {
        // 任务可能是在沙箱里提交的：BlockGuard 策略是 per-thread 且不继承，换到池线程就丢了，
        // 这里按"提交者"的沙箱补装一次（不在沙箱里时原样直通，零成本）。
        Runnable body = BlockGuardSandbox.wrap(task);
        return () -> {
            if (shutdown) {
                return;
            }
    ...
    private <T> Callable<T> wrapTask(Callable<T> task) {
        Callable<T> body = BlockGuardSandbox.wrap(task);
        return () -> {
            if (shutdown) {
