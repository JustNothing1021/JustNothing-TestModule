...
import com.justnothing.engine.v2.exception.ErrorCode;
import com.justnothing.engine.v2.exception.ParseException;
...
public class Lexer {
    ...
    /**
     * 读取数字字符（跳过下划线分隔符），返回纯数字字符串（不含 _）。
     */
    private String readDigits(boolean hex) {
        StringBuilder sb = new StringBuilder();
        while (!isAtEnd()) {
            char c = peek();
            if (c == '_') { advance(); continue; }
            if (hex ? isHexDigit(c) : Character.isDigit(c)) {
                sb.append(advance());
            } else {
                break;
            }
    ...
    private void readNumber() {
        ...
        // === Hex float / hex integer: 0x / 0X prefix ===
        if (peek() == '0' && !isAtEnd() && (peekNext() == 'x' || peekNext() == 'X')) {
            advance(); advance();
            String intPart = readDigits(true);
            boolean hasDot = false;
            String fracPart = "";
            if (!isAtEnd() && peek() == '.') {
                char afterDot = position + 1 < length ? source.charAt(position + 1) : '\0';
                if (afterDot != '.') { // 排除 .. 范围运算符
                    hasDot = true;
                    advance();
                    fracPart = readDigits(true);
                }
            }
            // 有 p/P → hex float
            if (!isAtEnd() && (peek() == 'p' || peek() == 'P')) {
                advance();
                boolean expNeg = match('-');
                if (!expNeg) match('+');
                String expStr = readDigits(false);
                String hexLiteral = "0x" + intPart + (hasDot ? "." + fracPart : "") + "p" + (expNeg ? "-" : "") + expStr;
                boolean isFloatSuffix = false;
                if (!isAtEnd()) {
                    char s = Character.toLowerCase(peek());
                    if (s == 'f') { advance(); isFloatSuffix = true; }
                    else if (s == 'd') advance();
                }
                try {
                    double value = Double.parseDouble(hexLiteral);
                    addToken(TokenType.LITERAL_DECIMAL, isFloatSuffix ? (double) (float) value : value, location);
                } catch (NumberFormatException e) {
                    throw error("Invalid hex float: " + hexLiteral, ErrorCode.LEXICAL_INVALID_NUMBER);
                }
                return;
            }
            // 无 p/P → hex integer
            boolean isLong = false;
            if (!isAtEnd() && Character.toLowerCase(peek()) == 'l') {
                advance();
                isLong = true;
            }
            try {
                long value = Long.parseLong(intPart, 16);
                addToken(isLong ? TokenType.LITERAL_LONG : TokenType.LITERAL_INTEGER, value, location);
            } catch (NumberFormatException e) {
                throw error("Invalid hex integer: 0x" + intPart, ErrorCode.LEXICAL_INVALID_NUMBER);
            }
            return;
        }
        ...
        if (peek() == '0' && !isAtEnd() && (peekNext() == 'b' || peekNext() == 'B')) {
            advance(); advance();
            String binStr = readDigits(false);
            boolean isLong = false;
            if (!isAtEnd() && Character.toLowerCase(peek()) == 'l') {
                advance();
                isLong = true;
        ...
        // === Octal: 0o / 0O ===
        if (peek() == '0' && !isAtEnd() && (peekNext() == 'o' || peekNext() == 'O')) {
            advance(); advance();
            String octStr = readDigits(false);
            boolean isLong = false;
            if (!isAtEnd() && Character.toLowerCase(peek()) == 'l') {
                advance();
                isLong = true;
        ...
        // === Decimal / float ===
        String intPart = readDigits(false);
        boolean hasDot = false;
        String fracPart = "";
        if (!isAtEnd() && peek() == '.') {
            char afterDot = position + 1 < length ? source.charAt(position + 1) : '\0';
        ...
        if (!isAtEnd() && (peek() == 'e' || peek() == 'E')) {
            ...
            boolean expNeg = match('-');
            if (!expNeg) match('+');
            expPart = (expNeg ? "-" : "") + readDigits(false);
        }

        String numberStr = intPart + (hasDot ? "." + fracPart : "") + (hasExponent ? "e" + expPart : "");
        ...
        if (!isAtEnd()) {
            ...
            if (suffix == 'f') { advance(); isFloat = true; }
            else if (suffix == 'l') { advance(); isLong = true; }
            else if (suffix == 'd') { advance(); isDouble = true; }
        }
        ...
        } else if (isDouble || hasDot || hasExponent) {
            try {
            ...
            while (pos < line.length()) {
                ...
                if (c == '$' && pos + 1 < line.length()) {
                    ...
                    if (next == '{') {
                        ...
                        pos = exprStart;
                        continue;
                    } else if (Character.isLetterOrDigit(next) || next == '_' || next == '$') {
                        // $var 形式
                        flushPending(parts, pendingText);
                        int varStart = pos + 1;
                        while (varStart < line.length()
                                && (Character.isLetterOrDigit(line.charAt(varStart)) || line.charAt(varStart) == '_' || line.charAt(varStart) == '$')) {
                            varStart++;
                        }
    ...
    private void readIdentifier() {
        ...
        // 贪心收集限定名：ID DOT ID DOT ID ...
        // 前瞻（不消费），如果 DOT 后面跟着 IDENTIFIER，就继续收集
        String qualifiedName = tryCollectQualifiedName(text);
        if (qualifiedName != null) {
            addToken(TokenType.QUALIFIED_NAME, qualifiedName, location);
        } else {
     ...
     * 尝试贪心收集限定名。
     * <p>
     * 从当前位置前瞻，如果看到 DOT + IDENTIFIER 序列，就消费并收集。
     * 返回完整的限定名字符串，或 null（如果不是限定名）。
     * </p>
     * <p>
     * 注意：只收集 ID DOT ID 部分，不收集后面的泛型参数 {@code <...>} 或方法调用 {@code (...)}。
     * 例如：
     * <ul>
     *   <li>{@code java.util.Map} → "java.util.Map"</li>
     *   <li>{@code System.out.println} → "System.out.println"</li>
     *   <li>{@code Map.Entry} → "Map.Entry"</li>
     *   <li>{@code x} → null（单个标识符不是限定名）</li>
     * </ul>
     * </p>
     */
    private String tryCollectQualifiedName(String firstPart) {
        // 至少需要一个 DOT + IDENTIFIER 才构成限定名
        if (isAtEnd() || peek() != '.') {
            return null;
        }

        // 前瞻：DOT 后面必须是 IDENTIFIER（字母/下划线开头）
        // 排除：1.5（数字）、..（范围运算符）、.class 等
        ...
        while (!isAtEnd() && peek() == '.') {
            // 前瞻 DOT 后面的字符
            int dotPos = position;
            if (dotPos + 1 >= length) break;

            char afterDot = source.charAt(dotPos + 1);

            // DOT 后面不是标识符起始字符 → 停止（如 x.5, x.., x( 等）
            if (!Character.isLetter(afterDot) && afterDot != '_' && afterDot != '$') break;

            // 消费 DOT
            advance();
            int beforeSegLen = result.length();
            result.append('.');
            ...
            // .class 不应并入限定名，让 parser 的 parsePostfix 处理
            String segment = result.substring(beforeSegLen + 1); // 去掉 '.'
            if (segment.equals("class")) {
                // 回退到消费 DOT 之前的位置
                position = dotPos;
                // 恢复 result 到消费前的长度
                result.setLength(beforeSegLen);
                break;
            }
    ...
    private void readOperator() {
        ...
        switch (c) {
            ...
            case '.':
                if (match('.')) {
                    if (match('.')) {
                        addToken(TokenType.DELIMITER_ELLIPSIS, location);
                    } else if (match('<')) {
